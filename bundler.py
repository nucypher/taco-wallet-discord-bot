        ...
        pimlico_dict.update(
            {
                ...
                "paymasterVerificationGasLimit": hex(
                    nucypher_dict["paymaster_verification_gas_limit"]
                ),
                "paymasterPostOpGasLimit": hex(
                    nucypher_dict["paymaster_post_op_gas_limit"]
                ),
...
class BundlerClient:
    ...
    def estimate_user_operation_gas(
        ...
        return self._make_bundler_request(
            "eth_estimateUserOperationGas",
            [user_op_dict, self.config.entry_point_address],
        )
    ...
    def send_user_operation(self, user_operation: UserOperation) -> Dict:
        ...
        user_op_dict = convert_user_operation_to_pimlico_format(user_operation)
        result = self._make_bundler_request(
            "eth_sendUserOperation", [user_op_dict, self.config.entry_point_address]
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "result" in result:
                    ...
                    error = result["error"]
                    logger.error(
                        f"Bundler error: {error.get('message', 'Unknown error')}"
                    )
                    return None
            else:
