...
class PorterSignatureService:
    ...
    def sign_user_operation(self, user_operation: UserOperation, context: str, discord_context: Dict) -> UserOperation:
        ...
        # Prepare Porter context with Discord signature data
        ...
        signatures = self._request_signatures(signing_request)

        # Return UserOperation with combined signature
        return self._create_signed_user_operation(user_operation, signatures)

    def _request_signatures(self, signing_request: UserOperationSignatureRequest) -> List[str]:
