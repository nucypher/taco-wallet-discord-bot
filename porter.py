...
class PorterSignatureService:
    ...
    async def sign_user_operation(
        self, user_operation: UserOperation, context: str
    ) -> UserOperation:
        ...
        porter_context = {
            ":bytes:message": discord_context["message_hex"],
    ...
    async def _request_signatures(
        self, signing_request: UserOperationSignatureRequest
    ) -> List[str]:
    ...
    def _create_signed_user_operation(
        self, user_operation: UserOperation, signatures: List[str]
    ) -> UserOperation:
