...
class TacoSmartWalletService:
    ...
    def __init__(self, config: SmartAccountConfig):
        ...
        logger.info(
            f"TACo smart wallet service initialized for {config.smart_account_address}"
        )
    ...
    def _validate_balance(self, amount_wei: int, amount_eth: float) -> None:
        ...
        smart_account_checksum = self.web3.to_checksum_address(
        ...
        if balance_wei < amount_wei:
            raise Exception(
                f"Insufficient balance: {balance_eth} ETH < {amount_eth} ETH"
            )
    ...
    def _optimize_gas_settings(self, user_operation):
        ...
        if gas_prices and "fast" in gas_prices:
            ...
            if "maxPriorityFeePerGas" in fast_prices:
                user_operation.max_priority_fee_per_gas = int(
                    fast_prices["maxPriorityFeePerGas"], 16
                )
        ...
        if gas_estimates:
            ...
            if "preVerificationGas" in gas_estimates:
                user_operation.pre_verification_gas = int(
                    gas_estimates["preVerificationGas"], 16
                )

        return user_operation

    def _format_transfer_result(
        self, bundler_result: Dict, recipient: str, amount_eth: float
    ) -> Dict:
