...
class DiscordInteractionHandler:
    ...
    def _generate_address_info(self, target_user: str) -> str:
        ...
        # Extract numeric user ID from mention if necessary
        user_id_match = re.match(r"<@!?(\d+)>", target_user)
        if user_id_match:
        ...
        try:
            ...
            address = compute_smart_account_address(user_id)

            return f"<@{user_id}> smart account: `{address}`"
